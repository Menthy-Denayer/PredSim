...
% Get number of bushings, added by Menthy
nBushings = 0;
for i = 0:forceSet.getSize()-1
    c_force_elt = forceSet.get(i);
    if strcmp(c_force_elt.getConcreteClassName(), 'BushingForce')
        nBushings = nBushings + 1;
    end
end

if(nBushings > 0)
    exportBushingForces = true;
else
    exportBushingForces = false;
end
...
if exportBushingForces
    nOutputs = nOutputs + 12*nBushings;                                         % 3 forces & 3 moments for 2 bodies
end
...
fprintf(fid, '#include <OpenSim/Simulation/Model/BushingForce.h>\n');           % Added Menthy
...
% Add bushing forces (added by Menthy)
for i = 0:forceSet.getSize()-1
    c_force = forceSet.get(i);
    
    if strcmp(char(c_force.getConcreteClassName()), 'BushingForce')
        
        c_force_obj = BushingForce.safeDownCast(c_force);
        c_force_name = c_force.getName();

        frame1_obj = c_force_obj.getFrame1();
        frame2_obj = c_force_obj.getFrame2();
        
        frame1_name = frame1_obj.getName();
        frame2_name = frame2_obj.getName();
        
        body1_name = strrep(string(frame1_obj.getSocket("parent").getConnecteePath()),"/bodyset/","");
        body2_name = strrep(string(frame2_obj.getSocket("parent").getConnecteePath()),"/bodyset/","");
        
        Transform1 = frame1_obj.findTransformInBaseFrame().T.getAsMat();
        Transform2 = frame2_obj.findTransformInBaseFrame().T.getAsMat();

        Rotation1 = frame1_obj.findTransformInBaseFrame().R.convertRotationToBodyFixedXYZ().getAsMat();
        Rotation2 = frame2_obj.findTransformInBaseFrame().R.convertRotationToBodyFixedXYZ().getAsMat();
        
        rotStiff = c_force_obj.get_rotational_stiffness().getAsMat();
        transStiff = c_force_obj.get_translational_stiffness().getAsMat();
        
        rotDamp = c_force_obj.get_rotational_damping().getAsMat();
        transDamp = c_force_obj.get_translational_damping().getAsMat();
        
        fprintf(fid,'\tOpenSim::BushingForce* %s;\n', c_force_name);
        
        % add transforms for both frames
        fprintf(fid,['\t%s = new OpenSim::BushingForce("%s", "%s", ' ...
            'Vec3(%.20f, %.20f, %.20f), Vec3(%.20f, %.20f, %.20f), ' ...
            '"%s", ' ...
            'Vec3(%.20f, %.20f, %.20f), Vec3(%.20f, %.20f, %.20f), ' ...
            'Vec3(%.20f, %.20f, %.20f), Vec3(%.20f, %.20f, %.20f), ' ...
            'Vec3(%.20f, %.20f, %.20f), Vec3(%.20f, %.20f, %.20f));\n'], ...
            c_force_name, c_force_name, body1_name, ...
            Transform1(1), Transform1(2), Transform1(3), ...
            Rotation1(1), Rotation1(2), Rotation1(3), ...
            body2_name, ...
            Transform2(1), Transform2(2), Transform2(3), ...
            Rotation2(1), Rotation2(2), Rotation2(3), ...
            transStiff(1), transStiff(2), transStiff(3), ...
            rotStiff(1), rotStiff(2), rotStiff(3), ...
            transDamp(1), transDamp(2), transDamp(3),...
            rotDamp(1), rotDamp(2), rotDamp(3));
...
% Add bushing forces (added by Menthy)
fprintf(fid, '\t/// Add bushing forces with moment correction to appliedBodyForces.\n');
b_count = 0;
for i = 0:forceSet.getSize()-1
    f_force_elt = forceSet.get(i);

    if strcmp(char(f_force_elt.getConcreteClassName()), "BushingForce")
        f_force_name = char(f_force_elt.getName());

        % Get the record values (forces/moments)
        fprintf(fid, '\tArray<osim_double_adouble> BForce_%d = %s->getRecordValues(*state);\n', b_count, f_force_name);
        fprintf(fid, '\tSpatialVec BushingForce_%d;\n', b_count);

        % Translational forces -> first 3 entries are Fx,Fy,Fz
        % Rotational moments -> last 3 entries are Tx,Ty,Tz
        fprintf(fid, '\tBushingForce_%i[0] = Vec3(BForce_%i[3], BForce_%i[4], BForce_%i[5]); // moment\n', b_count, b_count, b_count, b_count);
        fprintf(fid, '\tBushingForce_%i[1] = Vec3(BForce_%i[0], BForce_%i[1], BForce_%i[2]); // force\n', b_count, b_count, b_count, b_count);

        % Get the two connected bodies
        f_force_obj = BushingForce.safeDownCast(f_force_elt);
        body1Name = strrep(string(f_force_obj.getFrame1().getSocket("parent").getConnecteePath()),"/bodyset/","");
        body2Name = strrep(string(f_force_obj.getFrame2().getSocket("parent").getConnecteePath()),"/bodyset/","");

        % Get body indices
        fprintf(fid, '\tint b_idx1_%d = model->getBodySet().get("%s").getMobilizedBodyIndex();\n', b_count, body1Name);
        fprintf(fid, '\tint b_idx2_%d = model->getBodySet().get("%s").getMobilizedBodyIndex();\n', b_count, body2Name);
        
        % Get r vector from body locations in bushing frame
        fprintf(fid, '\tconst PhysicalFrame& body1_%d = model->getBodySet().get("%s");\n', ...
        b_count, body1Name);
        fprintf(fid, '\tconst PhysicalFrame& body2_%d = model->getBodySet().get("%s");\n', ...
                b_count, body2Name);
        
        % Distance vector between actual bushing application points
        fprintf(fid, '\tVec3 r2_%d = body2_%d.getPositionInGround(*state) - body1_%d.getPositionInGround(*state);\n', ...
        b_count, b_count, b_count);
        
        % Corrected moment for body2 (since applied on body 1, so need r x F correction for body 2)
        fprintf(fid, '\tSpatialVec BushingForce_body1_%d;\n', b_count);
        fprintf(fid, '\tBushingForce_body1_%d[1] = BushingForce_%d[1];\n', b_count, b_count);
        fprintf(fid, '\tBushingForce_body1_%d[0] = BushingForce_%d[0];\n', ...
                b_count, b_count);
        
        fprintf(fid, '\tSpatialVec BushingForce_body2_%d;\n', b_count);
        fprintf(fid, '\tBushingForce_body2_%d[1] = -BushingForce_%d[1];\n', b_count, b_count);
        fprintf(fid, '\tBushingForce_body2_%d[0] = -BushingForce_%d[0] + (r2_%d %% BushingForce_%d[1]);\n', ...
                b_count, b_count, b_count, b_count);

        % Add forces: body1 gets normal SpatialVec, body2 gets corrected
        fprintf(fid, '\tappliedBodyForces[b_idx1_%d] += BushingForce_body1_%d;\n', b_count, b_count);
        fprintf(fid, '\tappliedBodyForces[b_idx2_%d] += BushingForce_body2_%d;\n', b_count, b_count);
...
% bushing forces (added by Menthy)
if exportBushingForces
    fprintf(fid, '\t/// Bushing forces.\n');
    IO_BUSHINGs = struct();
    for i_bushing = 1:nBushings
        fprintf(fid, '\tfor (int i = 0; i < 3; ++i) res[0][i + nCoordinates + %i] = value<T>(BushingForce_body1_%s[1][i]);\n', count_acc, num2str(i_bushing-1));
        fprintf(fid, '\tfor (int i = 0; i < 3; ++i) res[0][i + nCoordinates + %i] = value<T>(BushingForce_body1_%s[0][i]);\n', count_acc+3, num2str(i_bushing-1));
        fprintf(fid, '\tfor (int i = 0; i < 3; ++i) res[0][i + nCoordinates + %i] = value<T>(BushingForce_body2_%s[1][i]);\n', count_acc+6, num2str(i_bushing-1));
        fprintf(fid, '\tfor (int i = 0; i < 3; ++i) res[0][i + nCoordinates + %i] = value<T>(BushingForce_body2_%s[0][i]);\n', count_acc+9, num2str(i_bushing-1));
        tmp = outputCount + count_acc;
        IO_BUSHINGs.("bushing_body1_" + i_bushing) = tmp:tmp+5;
        IO_BUSHINGs.("bushing_body2_" + i_bushing) = tmp+6:tmp+11;
        count_acc = count_acc + 12;
    end
    IO_indices.BUSHINGs = IO_BUSHINGs;
end
