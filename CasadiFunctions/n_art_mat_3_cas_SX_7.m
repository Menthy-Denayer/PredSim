...
%
% Data: 04/June/2026
% Added second derivative for joint stiffness computation by Menthy Denayer
...
diff2_mat_q = SX(nr_coefficients,6);
...
diff2_mat_q(:,1)=expo_q_all_nr(:,2).*expo_q_all_nr(:,3).*expo_q_all_nr(:,4).*expo_q_all_nr(:,5).*expo_q_all_nr(:,6).*expoVal(:,1).*(expoVal(:,1)-1).*(q_all_nr(:,1).^(expoVal(:,1)-2));
diff2_mat_q(:,2)=expo_q_all_nr(:,1).*expo_q_all_nr(:,3).*expo_q_all_nr(:,4).*expo_q_all_nr(:,5).*expo_q_all_nr(:,6).*expoVal(:,2).*(expoVal(:,2)-1).*(q_all_nr(:,2).^(expoVal(:,2)-2));
diff2_mat_q(:,3)=expo_q_all_nr(:,1).*expo_q_all_nr(:,2).*expo_q_all_nr(:,4).*expo_q_all_nr(:,5).*expo_q_all_nr(:,6).*expoVal(:,3).*(expoVal(:,3)-1).*(q_all_nr(:,3).^(expoVal(:,3)-2));
diff2_mat_q(:,4)=expo_q_all_nr(:,1).*expo_q_all_nr(:,2).*expo_q_all_nr(:,3).*expo_q_all_nr(:,5).*expo_q_all_nr(:,6).*expoVal(:,4).*(expoVal(:,4)-1).*(q_all_nr(:,4).^(expoVal(:,4)-2));
diff2_mat_q(:,5)=expo_q_all_nr(:,1).*expo_q_all_nr(:,2).*expo_q_all_nr(:,3).*expo_q_all_nr(:,4).*expo_q_all_nr(:,6).*expoVal(:,5).*(expoVal(:,5)-1).*(q_all_nr(:,5).^(expoVal(:,5)-2));
diff2_mat_q(:,6)=expo_q_all_nr(:,1).*expo_q_all_nr(:,2).*expo_q_all_nr(:,3).*expo_q_all_nr(:,4).*expo_q_all_nr(:,5).*expoVal(:,6).*(expoVal(:,6)-1).*(q_all_nr(:,6).^(expoVal(:,6)-2));
